...
void top_down(int i,int j,int k,bool l) {
    // a + b = c và bool l ???
    // guess : dp[i][j][k][l] xét idx i,j,k của a,b,c bool l = 1 nếu i+j > 10
	// minimum size
	if (!i&&!j&&!k&&!l) return; // nếu i = j = k = l = 0 return
	int imin1=i-1,jmin1=j-1,kmin1=k-1; // find next index
	if (imin1<0) imin1=0; if (jmin1<0) jmin1=0; if (kmin1<0) kmin1=0; // cant go further
	bool addi;
	if (!k) addi=true; // nếu k = 0 thì addi = 1
	else addi=false;
	if (a[i]-'0'+b[j]-'0'+l==c[k]-'0') {
		...
		if (d+b[j]-'0'+l==c[k]-'0') {
			// insert số vào idx i để hết nhớ
