...
/* declaration */
ll n,m,p;
ll a[MAXN],st[MAXN*4],add[MAXN*4],mul[MAXN*4];
...
void upd(ll op,ll l,ll r,ll val,ll node = 1,ll tl = 1,ll tr = n){
    ...
    if (l <= tl && tr <= r) {
        if (op == 1) (add[node] *= val) %= p, (st[node] *= val) %= p, (mul[node] *= val) %= p;
        else (add[node] += val) %= p, (st[node] += (tr-tl+1)*val % p) %= p;
        return;
    }
    push(node,tr-tl+1);
    ll tm = (tl+tr) >> 1;
    upd(op,l,r,val,left); upd(op,l,r,val,right);
    (st[node] = st[node << 1] + st[node << 1 | 1]) %= p;
}
ll query(ll l,ll r,ll node = 1,ll tl = 1,ll tr = n){
    ...
    push(node,tr-tl+1);
    ll tm = (tl+tr) >> 1;
    return (query(l,r,left) + query(l,r,right)) % p;
}
/* workspace */
signed main(){
